...
#[derive(Debug)]
pub struct SGXWitnessInputProducerDal<'a, 'c> {
...
#[sqlx(type_name = "sgx_witness_input_producer_job_status")]
pub enum SGXWitnessInputProducerJobStatus {
    ...
    /// [`SGXWitnessInputProducerStatus::Successful`] or [`SGXWitnessInputProducerStatus::Failed`].
    InProgress,
...
impl SGXWitnessInputProducerDal<'_, '_> {
    pub async fn create_sgx_witness_input_producer_job(
        &mut self,
        ...
        sqlx::query!(
            r#"
            INSERT INTO
                sgx_witness_input_producer_jobs (l1_batch_number, status, created_at, updated_at)
            VALUES
                ($1, $2, NOW(), NOW())
            ON CONFLICT (l1_batch_number) DO NOTHING
            "#,
            i64::from(l1_batch_number.0),
            SGXWitnessInputProducerJobStatus::Queued as SGXWitnessInputProducerJobStatus,
        )
        .instrument("create_sgx_witness_input_producer_job")
        .report_latency()
    ...
    pub async fn get_next_sgx_witness_input_producer_job(
        &mut self,
    ) -> sqlx::Result<Option<L1BatchNumber>> {
        let l1_batch_number = sqlx::query!(
            r#"
            UPDATE sgx_witness_input_producer_jobs
            SET
                status = $1,
                attempts = attempts + 1,
                updated_at = NOW(),
            ...
            WHERE
                l1_batch_number = (
                    SELECT
                        l1_batch_number
                    FROM
                        sgx_witness_input_producer_jobs
                    WHERE
                        status = $2
                        OR (
                            status = $1
            ...
            RETURNING
                sgx_witness_input_producer_jobs.l1_batch_number
            "#,
            SGXWitnessInputProducerJobStatus::InProgress as SGXWitnessInputProducerJobStatus,
            SGXWitnessInputProducerJobStatus::Queued as SGXWitnessInputProducerJobStatus,
            SGXWitnessInputProducerJobStatus::Failed as SGXWitnessInputProducerJobStatus,
            &JOB_PROCESSING_TIMEOUT,
            JOB_MAX_ATTEMPT,
        )
        .instrument("get_next_sgx_witness_input_producer_job")
        .report_latency()
    ...
    pub async fn get_sgx_witness_input_producer_job_attempts(
        &mut self,
        l1_batch_number: L1BatchNumber,
    ...
    pub async fn mark_job_as_successful(
        ...
        sqlx::query!(
            ...
            WHERE
                l1_batch_number = $2
            "#,
            SGXWitnessInputProducerJobStatus::Successful as SGXWitnessInputProducerJobStatus,
            i64::from(l1_batch_number.0),
    ...
    pub async fn mark_job_as_failed(
        ...
        let attempts = sqlx::query!(
            ...
            WHERE
                ...
                AND status != $5
            RETURNING
                sgx_witness_input_producer_jobs.attempts
            "#,
            SGXWitnessInputProducerJobStatus::Failed as SGXWitnessInputProducerJobStatus,
            i64::from(l1_batch_number.0),
            duration_to_naive_time(started_at.elapsed()),
...
impl SGXWitnessInputProducerDal<'_, '_> {
    pub async fn delete_all_jobs(&mut self) -> sqlx::Result<()> {
        sqlx::query!(
            r#"
            DELETE FROM sgx_witness_input_producer_jobs
            "#
        )
