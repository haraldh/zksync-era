...
use zksync_dal::{sgx_witness_input_producer_dal::JOB_MAX_ATTEMPT, ConnectionPool, Core, CoreDal};
...
#[derive(Debug)]
pub struct SGXWitnessInputProducer {
    connection_pool: ConnectionPool<Core>,
    l2_chain_id: L2ChainId,
    object_store: Arc<dyn ObjectStore>,
}

pub struct SGXBlockOutputWithProofs {
    pub inner: BlockOutputWithProofs,
}

impl From<PrepareBasicCircuitsJob> for SGXBlockOutputWithProofs {
    fn from(value: PrepareBasicCircuitsJob) -> Self {
        let mut logs: Vec<TreeLogEntryWithProof> = vec![];
        for path in value.into_merkle_paths() {
            let merkle_path = path.merkle_paths.iter().map(|x| x.into()).collect();
            let base: TreeLogEntry = {
                if !path.is_write {
                    if path.leaf_enumeration_index == 0 {
...
impl SGXWitnessInputProducer {
    pub async fn new(
        connection_pool: ConnectionPool<Core>,
        store_factory: &ObjectStoreFactory,
        l2_chain_id: L2ChainId,
    ) -> anyhow::Result<Self> {
        Ok(SGXWitnessInputProducer {
            connection_pool,
    ...
    fn process_job_impl(
        ...
        let blob: PrepareBasicCircuitsJob =
            rt_handle
        ...
        let mut connection = rt_handle
            ...
            .context("failed to get connection for SGXWitnessInputProducer")?;
        ...
        let (mut vm, storage_view) =
            create_vm(rt_handle.clone(), l1_batch_number, connection, l2_chain_id)
                .context("failed to create vm for SGXWitnessInputProducer")?;
        ...
        for (miniblock_data, next_miniblock_data) in miniblocks_data {
            ...
            for tx in &miniblock_data.txs {
                ...
                execute_tx(tx, &mut vm)
                    .context("failed to execute transaction in SGXWitnessInputProducer")?;
        ...
        tracing::info!(
            "SGXWitnessInputProducer took {:?} for L1BatchNumber {}",
            started_at.elapsed(),
            l1_batch_number.0
        );
...
#[async_trait]
impl JobProcessor for SGXWitnessInputProducer {
    type Job = L1BatchNumber;
    type JobId = L1BatchNumber;
    type JobArtifacts = WitnessBlockState;
    const SERVICE_NAME: &'static str = "sgx_witness_input_producer";
    ...
    async fn get_next_job(&self) -> anyhow::Result<Option<(Self::JobId, Self::Job)>> {
        ...
        let l1_batch_to_process = connection
            ...
            .get_next_sgx_witness_input_producer_job()
            .await
    ...
    async fn save_result(
        ...
        let object_path = self
            ...
            .await
            .context("failed to upload artifacts for SGXWitnessInputProducer")?;
        METRICS
            .upload_input_time
        ...
        let mut connection = self
            ...
            .connection()
            .await
            .context("failed to acquire DB connection for SGXWitnessInputProducer")?;
        let mut transaction = connection
            .start_transaction()
            .await
            .context("failed to acquire DB transaction for SGXWitnessInputProducer")?;
        transaction
            .sgx_witness_input_producer_dal()
            .mark_job_as_successful(job_id, started_at, &object_path)
            .await
            .context("failed to mark job as successful for SGXWitnessInputProducer")?;
        transaction
            .commit()
            .await
            .context("failed to commit DB transaction for SGXWitnessInputProducer")?;
    ...
    async fn get_job_attempts(&self, job_id: &L1BatchNumber) -> anyhow::Result<u32> {
        ...
        connection
            .sgx_witness_input_producer_dal()
            .get_sgx_witness_input_producer_job_attempts(*job_id)
            .await
            .map(|attempts| attempts.unwrap_or(0))
            .context("failed to get job attempts for SGXWitnessInputProducer")
    }
}
